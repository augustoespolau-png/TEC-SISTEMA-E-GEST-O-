-- 054 · Corrige o vínculo da foto ao desvio no caminho relacional.
--
-- O hot path de REGISTRAR_DESVIO grava diretamente em produto_desvios. A
-- versão anterior desta RPC procurava o desvio apenas no snapshot JSON legado
-- (auditoria_produto_estado.registros), por isso o upload terminava mas a foto
-- era rejeitada com "Desvio não encontrado nessa parede".

create or replace function private.qualidade_segmento_storage(p_valor text)
returns text
language sql
immutable
set search_path = ''
...
begin
  ...
  -- O primeiro segmento do objeto é fixo; o segundo é o proprietário. Os
  -- demais segmentos são os IDs sanitizados usados por caminhoAnexoAuditoria.
  if array_length(string_to_array(storage_path, '/'), 1) <> 6
     ...
     or split_part(storage_path, '/', 5) <> private.qualidade_segmento_storage(p_parede)
     or storage_path like '%..%' then
    raise exception 'Caminho de anexo inválido';
  end if;

  -- REGISTRAR_DESVIO e esta RPC podem chegar por requests diferentes, mas o
  -- desvio já está confirmado no relacional quando esta operação começa.
  -- Portanto o vínculo deve ser resolvido pela mesma fonte que a consulta usa,
  -- e não pelo snapshot JSON que não é atualizado no hot path.
...
'Vincula fotos de desvios no relacional, valida o caminho privado do Storage e não depende do snapshot legado.';
